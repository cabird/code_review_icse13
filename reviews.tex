...
\item[p.2] Please \hl{describe the initial in-field observations}.

\item[p.3] was there a \hl{Hawthorne effect to the ``thought aloud'' portion} of the work?

\item[p.4] top left column - please \hl{show a sample card}?

\item[p.4] top left - is there not possible \hl{bias from the first author sorting the
cards} to form initial themes?

\item[p.4] middle of left column - \hl{how time consuming were the card sorts}?
...
\item[pp.] There are some typos and minor grammatical issues - a thorough proofread of the
paper will remedy this

\item[p.9] need text under section VII header
...
way to carry out axial coding. \hl{It might be helpful to readers to connect these
...
\AB{I am not mentioning the two ``nits'' already fixed}
